...
class ir_basicblock : public printable {
    ...
    std::string name;
    int id;
    std::vector<std::shared_ptr<ir_instr>> instructions;
public:
    ...
    std::shared_ptr<ir_instr> back() const { return instructions.back(); }
    const std::vector<std::shared_ptr<ir_instr>>& get_instructions() const { return instructions; }
};
